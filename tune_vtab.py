...
def find_best_lrwd(files, data_name):
    ...
    for f in files:
        try:
            ...
            val_result = results_dict[f"epoch_{epoch}"]["classification"][t_name][
                "top1"
            ]
...
def train(cfg, args, final_runs):
    ...
    train_loader, val_loader, test_loader = get_loaders(cfg, logger, final_runs)
    ...
    if train_loader:
        acc = trainer.train_classifier(train_loader, val_loader, test_loader)
...
def get_lrwd_range(args):
    ...
    if args.train_type == "finetune":
        ...
        lr_range = [50.0, 25.0, 10.0, 5.0, 2.5, 1.0, 0.5, 0.25, 0.1, 0.05]
        wd_range = [0.01, 0.001, 0.0001, 0.0]
        ...
        lr_range = [0.05, 0.025, 0.01, 0.5, 0.25, 0.1, 1.0, 2.5, 5.0]
        wd_range = [0.01, 0.001, 0.0001, 0.0]
...
def main(args):
    ...
    if args.fast:
        ...
        for wd in sorted(wd_range, reverse=True):
            try:
                cfg = setup(args, best_lr, wd, final_runs=False)
            except ValueError:
                # already ran
                continue
            acc = train(cfg, args, final_runs=False)
        ...
        for lr in sorted(lr_range, reverse=True):
            try:
                cfg = setup(args, lr, best_wd, final_runs=False)
            except ValueError:
                # already ran
                continue
            acc = train(cfg, args, final_runs=False)
    ...
    output_dir = os.path.join(cfg.OUTPUT_DIR.split("/lr")[0], "summary")
    PathManager.mkdirs(output_dir)

    print("-" * 20)
    print(f"Best lr: {best_lr}, Best wd: {best_wd}, Best acc: {best_acc}")
    # save the best lr and wd
    with open(os.path.join(output_dir, f"best_lrwd_acc{best_acc:.3f}.txt"), "w") as f:
        f.write(f"Best lr: {best_lr}, Best wd: {best_wd}")
    print(
    ...
    for run_idx, seed in enumerate(random_seeds):
        try:
            ...
            # already ran
            continue
        acc = train(cfg, args, final_runs=True)

        accs.append(acc)

    print("-" * 20)
    print(f"Mean acc: {np.mean(accs)}")
    print(f"Std acc: {np.std(accs)}")
    saved = {
    ...
    with open(os.path.join(output_dir, f"seed_acc{np.mean(accs):.3f}.txt"), "w") as f:
        json.dump(saved, f)
    print(
        f"\033[92mSaving mean and std acc to {os.path.join(output_dir, f'seed_acc{np.mean(accs):.3f}.txt')}\033[0m"
    )
    print("-" * 20)

...
def compute_f1(
    ...
    f1 = {}
    f1["micro"] = f1_score(
        y_true=multihot_targets, y_pred=predict_labels, average="micro"
    )
    f1["samples"] = f1_score(
        y_true=multihot_targets, y_pred=predict_labels, average="samples"
    )
    f1["macro"] = f1_score(
        y_true=multihot_targets, y_pred=predict_labels, average="macro"
    )
    f1["none"] = f1_score(y_true=multihot_targets, y_pred=predict_labels, average=None)
...
def get_best_f1_scores(
    ...
    thrs = np.linspace(end, 0.95, int(np.round((0.95 - end) / 0.05)) + 1, endpoint=True)
    f1_micros = []
    f1_macros = []
    f1_samples = []
    f1_none = []

    ...
    def build_backbone(self, prompt_cfg, cfg, adapter_cfg, load_pretrain, vis):
        ...
        if transfer_type == "partial-1":
            ...
            for k, p in self.enc.named_parameters():
                if (
                    "transformer.encoder.layer.{}".format(total_layer - 1) not in k
            ...
            for k, p in self.enc.named_parameters():
                if (
                    "transformer.encoder.layer.{}".format(total_layer - 1) not in k
                    and "transformer.encoder.layer.{}".format(total_layer - 2) not in k
                    and "transformer.encoder.encoder_norm" not in k
                ):  # noqa
                    p.requires_grad = False
            ...
            for k, p in self.enc.named_parameters():
                if (
                    "transformer.encoder.layer.{}".format(total_layer - 1) not in k
                    and "transformer.encoder.layer.{}".format(total_layer - 2) not in k
                    and "transformer.encoder.layer.{}".format(total_layer - 3) not in k
                    and "transformer.encoder.layer.{}".format(total_layer - 4) not in k
                    and "transformer.encoder.encoder_norm" not in k
                ):  # noqa
                    p.requires_grad = False
            ...
            nn.init.normal_(self.enc.transformer.embeddings.cls_token, std=1e-6)
...
class Swin(ViT):
    ...
    def build_backbone(self, prompt_cfg, cfg, adapter_cfg, load_pretrain, vis):
        ...
        if transfer_type == "partial-1":
            ...
            for k, p in self.enc.named_parameters():
                if (
                    "layers.{}.blocks.{}".format(total_layer - 1, total_blocks - 1)
                    not in k
                    and "norm.weight" != k
                    and "norm.bias" != k
                ):  # noqa
            ...
            for k, p in self.enc.named_parameters():
                if (
                    "layers.{}".format(total_layer - 1) not in k
                    and "norm.weight" != k
                    and "norm.bias" != k
                ):  # noqa
                    p.requires_grad = False
            ...
            for k, p in self.enc.named_parameters():
                if (
                    "layers.{}".format(total_layer - 1) not in k
                    and "layers.{}.blocks.{}".format(total_layer - 2, total_blocks - 1)
                    not in k
                    and "layers.{}.blocks.{}".format(total_layer - 2, total_blocks - 2)
                    not in k
                    and "layers.{}.downsample".format(total_layer - 2) not in k
                    and "norm.weight" != k
                    and "norm.bias" != k
                ):  # noqa
                    p.requires_grad = False
...
class SSLViT(ViT):
    ...
    def build_backbone(self, prompt_cfg, cfg, adapter_cfg, load_pretrain, vis):
        ...
        if transfer_type == "partial-1":
            ...
            for k, p in self.enc.named_parameters():
                if (
                    "blocks.{}".format(total_layer - 1) not in k
                    and "fc_norm" not in k
                    and k != "norm"
                ):  # noqa
                    p.requires_grad = False
        elif transfer_type == "partial-2":
            ...
            for k, p in self.enc.named_parameters():
                if (
                    "blocks.{}".format(total_layer - 1) not in k
                    and "blocks.{}".format(total_layer - 2) not in k
                    and "fc_norm" not in k
                    and k != "norm"
                ):  # noqa
                    p.requires_grad = False
            ...
            for k, p in self.enc.named_parameters():
                if (
                    "blocks.{}".format(total_layer - 1) not in k
                    and "blocks.{}".format(total_layer - 2) not in k
                    and "blocks.{}".format(total_layer - 3) not in k
                    and "blocks.{}".format(total_layer - 4) not in k
                    and "fc_norm" not in k
                    and k != "norm"
                ):  # noqa
                    p.requires_grad = False
            ...
            for k, p in self.enc.named_parameters():
                if (
                    "prompt" not in k
                    and "patch_embed.proj.weight" not in k
                    and "patch_embed.proj.bias" not in k
                ):
                    p.requires_grad = False

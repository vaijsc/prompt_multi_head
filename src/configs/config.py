...
_C.ATTRIBUTION_TYPE = "general"  # one of "specific", "general" for attribution results
_C.ATTRIBUTION_INTEGRATED_METHOD = "pytorch_gradcam"  # one of "ig", "noise_tunnel" "occlusion" "layer_gradcam" for attribution results
...
_C.MODEL.TRANSFER_TYPE = (
    "linear"  # one of linear, end2end, prompt, adapter, side, partial-1, tinytl-bias
)
_C.MODEL.WEIGHT_PATH = ""  # if resume from some checkpoint file
_C.MODEL.SAVE_CKPT = False
_C.MODEL.SAVE_CKPT_FINALRUNS = False
...
_C.MODEL.PROMPT.DEEP = (
    False  # "whether do deep prompt or not, only for prepend location"
)


_C.MODEL.PROMPT.NUM_DEEP_LAYERS = (
    None  # if set to be an int, then do partial-deep prompt tuning
)
_C.MODEL.PROMPT.REVERSE_DEEP = (
    False  # if to only update last n layers, not the input layer
)
_C.MODEL.PROMPT.DEEP_SHARED = (
    False  # if true, all deep layers will be use the same prompt emb
)
_C.MODEL.PROMPT.FORWARD_DEEP_NOEXPAND = (
    False  # if true, will not expand input sequence for layers without prompt
)
...
# ----------------------------------------------------------------------
_C.MODEL.PROMPT.ADAPTIVE = False
_C.MODEL.PROMPT.DROPOUT_MLP = 0.1
_C.MODEL.PROMPT.ACTIVATION = "relu"
_C.MODEL.PROMPT.HIDDEN_DIM = 8
_C.MODEL.PROMPT.CONV = True
_C.MODEL.PROMPT.KERNEL = 2
_C.MODEL.PROMPT.PADDING = 0
_C.MODEL.PROMPT.VPT_DEPTH = 1
_C.MODEL.PROMPT.LEARNABLE_SCALE = False
_C.MODEL.PROMPT.CHANNELWISE = True
_C.MODEL.PROMPT.FEAT_PROJECTOR = True
_C.MODEL.PROMPT.SHARE_PROJECTOR = True
...
_C.SOLVER.BIAS_MULTIPLIER = 1.0  # for prompt + bias

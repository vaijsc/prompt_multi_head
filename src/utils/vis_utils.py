...
def get_mean_accuracy(job_path, data_name):
    ...
    v_matrix = confusion_matrix(
        val_data["targets"], np.argmax(val_data["joint_logits"], 1)
    )
    t_matrix = confusion_matrix(
        test_data["targets"], np.argmax(test_data["joint_logits"], 1)
    )
    return (
        np.mean(v_matrix.diagonal() / v_matrix.sum(axis=1)) * 100,
        np.mean(t_matrix.diagonal() / t_matrix.sum(axis=1)) * 100,
    )
...
def get_training_data(job_path, model_type, job_root):
    ...
    for line in lines:  # , leave=False):
        ...
        if "Total Parameters: " in line:
            ...
            gradiented_params = int(
                line.split("Gradient Parameters: ")[-1].split("\n")[0]
            )
...
def get_time(file):
    ...
    return (
        datetime.timedelta(seconds=(end_time - start_time).total_seconds()),
        np.mean(per_batch),
        np.mean(per_batch_train),
    )
...
def get_df(files, model_type, root, is_best=True, is_last=True, max_epoch=300):
    ...
    for job_path in tqdm(files, desc=model_type):
        train_loss, eval_results, meta_dict, (v_top1, t_top1) = get_training_data(
            job_path, model_type, root
        )
...
def average_df(df, metric_names=["l-val_top1", "l-val_base_top1"], take_average=True):
    ...
    hp_names = [
        c
        for c in df.columns
        if c not in ["data", "feature", "type", "file", "best_epoch"] + metric_names
    ]
...
def display_results(
    df, sorted_cols=["data", "feature", "type", "l-val_top1"], max_num=1
):

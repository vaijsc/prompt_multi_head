    ...
    def _tensors_to_cast():
        ...
        for tensor_name, tensor_dtype in output_tensors.items():
            if isinstance(tensor_dtype, tuple) and len(tensor_dtype) == 2:
                ...
                raise ValueError(
                    "Values of the output_tensors dictionary must be "
...
class ImageData(ImageDataInterface):
    ...
    def get_tf_data(
        ...
        data = self._get_dataset_split(
            split_name=split_name, shuffle_files=shuffle_files
        )
    ...
    def _preprocess_and_batch_data(
        self,
        data,
        batch_size,
        drop_remainder=True,
        ...
        # features etc.).
        base_preprocess_fn = compose_preprocess_fn(
            self._image_decoder, self._base_preprocess_fn
        )
        ...
        if pairwise_mix_fn is not None:
            data = tf.data.Dataset.zip((data, data.skip(1))).map(
                pairwise_mix_fn, self._num_preprocessing_threads
            )

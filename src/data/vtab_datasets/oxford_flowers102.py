...
class OxfordFlowers102Data(base.ImageTfdsData):
    ...
    def __init__(self, data_dir=None, train_split_percent=None):
        ...
        if train_split_percent:
            tfds_splits = {
                "train": "train[:{s}%]+validation[:{s}%]".format(s=train_split_percent),
                "val": "train[-{s}%:]+validation[-{s}%:]".format(s=train_split_percent),
            ...
            num_samples_splits = {
                "train": (((train_count + val_count) // 100) * train_split_percent),
                "val": (

...
def _count_preprocess_fn(x):
    return {"image": x["image"], "label": tf.size(x["objects"]["size"]) - 3}


def _count_cylinders_preprocess_fn(x):
    # Class distribution:

    num_cylinders = tf.reduce_sum(tf.cast(tf.equal(x["objects"]["shape"], 2), tf.int32))
...
_TASK_DICT = {
    "count_all": {"preprocess_fn": _count_preprocess_fn, "num_classes": 8},
    "count_cylinders": {

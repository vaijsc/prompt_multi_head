...
def setup(args):
    ...
    # Setup lr
    cfg.SOLVER.BASE_LR = cfg.SOLVER.BASE_LR * cfg.DATA.BATCH_SIZE / 256
...
def get_loaders(cfg, logger):
    ...
    if cfg.DATA.NAME.startswith("vtab-") or cfg.DATA.FULL_TRAIN:
...
def train(cfg, args):
    ...
    if train_loader:
        test_acc = trainer.train_classifier(train_loader, val_loader, test_loader)
    else:
